...
public class AssigningDL {
       	...
       	Optional<DriverInfo> driver1=driverRepo.findById(eachCab.getDriverId());
            ...
            if(tripCabAssigned == null && totalSeats >= selected ) {
                ...
                for( TripCabInfo eachTrip:tripCabAssigned) {
                    ...
                    if(remainingSeats >= selected && dest.equals(destination)&& time1.equals(time) && (eachTrip.getStatus().equalsIgnoreCase("Assigned"))) {
                        availableCabs.add(new AssigningBO(eachTrip.getCabNumber(), driver.getDriverName(), driver.getDriverNumber(), destination, time1, eachTrip.getTotalSeats(),eachTrip.getRemainingSeats() , eachTrip.getAllocatedSeats(),null,null,null));
                    }
                }
	...
	// To update and save in the Trip cab info , update in Booking Request table
      ...
      public AssigningBO storeTrip(AssigningBO info) {
		...
		if(trip!=null) {
			...
			TripCabInfo newTrip=new TripCabInfo();
			newTrip.setTripCabId((int)tripRepo.count()+1);
	...
	public boolean updateRequest( List<BookingRequest> req, Integer tripId) {
